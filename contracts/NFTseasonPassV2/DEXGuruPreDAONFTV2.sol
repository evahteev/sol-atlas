...
contract DEXGuruPreDAONFTV3 is ERC721Enumerable, FunctionsClient, ConfirmedOwner {
    using Strings for uint256;
    ...
    uint256 private constant MINTING_END_TIME = 1716201895;     // 04/01/2022 @ 9:00am (PST)
    ...
    ) ERC721('GURU Season Pass NFT', 'SeasonPass') FunctionsClient(router) ConfirmedOwner(msg.sender) {}
	...
	/// @param merkleProof Merkle proof
    function checkWhitelistUser(address account, bytes32[] memory merkleProof) public view returns (bool) {
    ...
    /// Update Merkle Root via Chainlink Functions
    ...
    /// Callback function to update Merkle Root
    /// docs: https://docs.chain.link/docs/chainlink-functions
    function fulfillRequest(
        ...
        if (err.length == 0) {
            _updateMerkleRoot(response);
         }
        s_lastError = err;
        emit Response(requestId, response, s_lastError);
    }

    function _updateMerkleRoot(bytes memory newRoot) internal {
