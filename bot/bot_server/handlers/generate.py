...
async def events_handler(message: types.Message, state: FSMContext) -> None:
    try:
        ...
        try:
            if '/generate@bmeme_stage_bot' in message.text:
                prompt = message.text.split("/generate@bmeme_stage_bot ")[1]
            else:
                prompt = message.text.split("/generate ")[1]
        except IndexError:
            await message.answer("Please provide a prompt")
            return
        resulting_variables = [{'name': 'prompt', 'value': prompt, 'type': 'String'}]
        ...
        async with httpx.AsyncHTTPTransport() as transport:
            ...
            process_instance = await camunda_client.get_process_instances(
                params=ProcessInstanceQuerySchema(
                    ...
                    process_definition_key="generate_art_and_vote"
                )
            )
