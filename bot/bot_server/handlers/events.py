...
async def events_handler(message: types.Message, state: FSMContext) -> None:
    try:
        ...
        process_definitions = await get_process_definitions(prefix="events",
        ...
        await message.answer("Event actions:", reply_markup=start_process_keyboard(process_definitions))
...
async def process_instance_started_callback(callback_query: types.CallbackQuery, state: FSMContext,
    ...
    if process_instance_id:
        ...
        tasks = await fetch_camunda_tasks(session, process_instance_id=process_instance_id)
        if tasks:
