...
exports.parsePumpTransaction = parsePumpTransaction;
const bs58_1 = __importDefault(require("bs58"));
const pump_deserializer_1 = require("./pump-deserializer");
 ...
 */
async function parsePumpTransaction(tx) {
    const trades = [];
    ...
    for (let i = 0; i < message.instructions.length; i++) {
        ...
        if (programId !== constants_solana_1.PUMP_PROGRAM_ID) {
            continue;
        }
        ...
        const events = await extractTradeEventsRecursively(innerInstructions.instructions, message.accountKeys, tx, i, instruction);
        trades.push(...events);
    }
    return trades;
}
 ...
 */
async function extractTradeEventsRecursively(instructions, accountKeys, tx, topLevelIndex, parentInstruction) {
    const trades = [];
    for (const innerIx of instructions) {
        ...
        if (innerProgramId === constants_solana_1.PUMP_PROGRAM_ID) {
            try {
                // Decode base58 instruction data
                const data = bs58_1.default.decode(innerIx.data);
                const tradeEvent = (0, pump_deserializer_1.deserializePumpTradeEvent)(Buffer.from(data));
                if (tradeEvent) {
                    const trade = convertPumpEventToTrade(tradeEvent, tx, topLevelIndex, parentInstruction, accountKeys);
                    // Apply whitelist filter: only save trades involving whitelisted tokens
                    if ((0, whitelist_1.hasWhitelistedToken)(trade.token_addresses)) {
                        trades.push(trade);
                    }
                }
            }
 ...
 */
function convertPumpEventToTrade(event, tx, instructionIndex, instruction, accountKeys) {
    ...
    try {
        if (instruction.accounts && instruction.accounts.length > constants_solana_1.PUMP_ACCOUNT_INDICES.BONDING_CURVE) {
            const accountIndex = instruction.accounts[constants_solana_1.PUMP_ACCOUNT_INDICES.BONDING_CURVE];
    ...
    const tokenAddresses = event.isBuy
        ? [constants_solana_1.NATIVE_SOL_ADDRESS, event.mint.toBase58()]
        : [event.mint.toBase58(), constants_solana_1.NATIVE_SOL_ADDRESS];
    ...
    const swapType = (tokenOut === constants_solana_1.NATIVE_SOL_ADDRESS || tokenOut === constants_solana_1.SOLANA_USDC_ADDRESS) ? 'sell' : 'buy';
    const trade = {
        ...
        lp_token_address: '',
        factory_address: constants_solana_1.PUMP_PROGRAM_ID,
        // Prices
        token_prices: [[price]],
 ...
 */
function calculatePrice(solAmount, tokenAmount) {
    if (tokenAmount === 0n) {
        return '0';
    }
