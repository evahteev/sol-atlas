...
exports.parseJupiterTransaction = parseJupiterTransaction;
const bs58_1 = __importDefault(require("bs58"));
const jupiter_deserializer_1 = require("./jupiter-deserializer");
 ...
 */
async function parseJupiterTransaction(tx) {
    const trades = [];
    ...
    for (let i = 0; i < message.instructions.length; i++) {
        ...
        if (programId !== constants_solana_1.JUPITER_V6_PROGRAM_ID && programId !== constants_solana_1.JUPITER_V4_PROGRAM_ID) {
            continue;
        }
        ...
        for (const innerIx of innerInstructions.instructions) {
            ...
            if (innerProgramId === constants_solana_1.JUPITER_V6_PROGRAM_ID || innerProgramId === constants_solana_1.JUPITER_V4_PROGRAM_ID) {
                try {
                    ...
                    const swapEvent = (0, jupiter_deserializer_1.deserializeJupiterSwapEvent)(Buffer.from(data));
                    if (swapEvent) {
 ...
 */
function convertJupiterEventToTrade(event, tx, topLevelIndex) {
    // Calculate price
    ...
    const amount_native = (tokenIn === constants_solana_1.NATIVE_SOL_ADDRESS)
        ? event.inputAmount.toString()
        : (tokenOut === constants_solana_1.NATIVE_SOL_ADDRESS)
    ...
    const trade = {
        ...
        lp_token_address: '',
        factory_address: constants_solana_1.JUPITER_V6_PROGRAM_ID,
        token_reserves: [],
