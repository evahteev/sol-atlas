...
import { CONFIG_SOLANA } from './config-solana';
 ...
 */
export function heightToSlot(height: number): number {
    const APPROXIMATE_OFFSET = 22_000_000;
...
export async function getLastProcessedBlock(
    ...
    try {
        ...
        const data = await result.json<{ state_value: string }>();
...
export async function getStartingBlock(
    ...
    const startHeight = CONFIG_SOLANA.START_HEIGHT;
