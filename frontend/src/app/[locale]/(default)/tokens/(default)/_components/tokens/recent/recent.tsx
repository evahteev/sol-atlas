...
import { fetchTokensRecent } from '@/actions/tokens'
...
type TokensExplorerRecentProps = {
  className?: string
  chains: ChainModel[]
}
...
export const TokensExplorerRecent: FC<TokensExplorerRecentProps> = ({ className, chains }) => {
  const [currentFilter, setCurrentFilter] = useState<Record<string, string>>({})
  const { data, isLoading } = useQuery({
    queryKey: ['fetchTokensRecent', currentFilter['network']],
    queryFn: () =>
      fetchTokensRecent({
  ...
  return (
    <>
      <Message className={styles.comment}>
        ...
        &gt;$50,000 in liquidity. Tokens below are also under Degen Mode as they are not on verified
        token lists, remember to DYOR and verify contract addresses.
      </Message>
