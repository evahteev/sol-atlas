...
import { WarehouseDashboardChartTreemap } from './WarehouseDashboardChartTreemap'

import styles from './WarehouseDashboardChart.module.scss'

const SUPPORTED_CHARTS = ['line', 'area', 'column', 'bar', 'pie', 'heatmap', 'scatter', 'treemap']
...
export const WarehouseDashboardChart: FC<{
  ...
  if (!SUPPORTED_CHARTS.includes(type)) {
    return <Message type="danger" text={`Chart type ${type} is currently not supported`} />
  }
    ...
    return (
      <div
        ...
        <div className={styles.wrapper}>
          <WarehouseDashboardChartTreemap
  ...
  groupedChartEntries.forEach(([groupedDataKey], setIdx) => {
    yAxisColumns.forEach((column, colIdx) => {
      ...
      const yAxisScale =
        visualization.options.yAxis[yAxisId].type === 'logarithmic' ? 'symlog' : undefined
