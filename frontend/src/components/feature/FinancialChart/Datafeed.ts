import { KLineData } from 'klinecharts'

import { fetchFinancialChartSearch, fetchTradingHistory } from '@/actions/tokens'
import { DatafeedSubscribeCallback, Period, SymbolInfo } from '@/lib/dash-guru'
import { convertTimespan } from '@/utils/dates'

type SubscribeCallback = (period: Period, callback: DatafeedSubscribeCallback) => void
type UnsubscribeCallback = (ticker: string, resolution: number) => void

export class FinancialChartDatafeed {
  private _prevData?: { symbol: SymbolInfo; period: Period }
  private _onSubscribe?: SubscribeCallback
  private _onUnsubscribe?: UnsubscribeCallback

  constructor({
    onSubscribe,
    onUnsubscribe,
  }: {
    onSubscribe: SubscribeCallback
    onUnsubscribe: UnsubscribeCallback
  }) {
    this._onSubscribe = onSubscribe
    this._onUnsubscribe = onUnsubscribe
  }

  /**
   * Fuzzy search symbols
   * Triggered when the search box is entered
   * Returns an array of symbol information
   */
  async searchSymbols(search?: string): Promise<SymbolInfo[]> {
    return ((search ? await fetchFinancialChartSearch(search) : []) ?? []).map(
      (data: {
        symbol: string
        full_name: string
        description: string
        exchange: string
        type: string
        ticker: string
      }) => {
        return {
          ticker: data.ticker,
          name: data.description,
          shortName: data.full_name,
          market: 'crypto',
          exchange: data.exchange,
          priceCurrency: 'USD',
          type: data.type,
          logo: 'data:image/png;base64,',
        }
      }
    )
  }

  /**
   * Pull historical k-line data
   * Triggered when the symbol and period change
   *
   * Returns the symbol k-line data array
   */
  async getHistoryKLineData(
    symbol: SymbolInfo,
    period: Period,
    from: number,
    to: number
  ): Promise<KLineData[]> {
    return await fetchTradingHistory(
      symbol.ticker,
      `${convertTimespan(period.multiplier, period.timespan, 'minute')}`,
      from,
      to
    ).then(
      (chartData: {
        t: number[]
        o: number[]
        h: number[]
        l: number[]
        c: number[]
        v: number[]
        liq: number[]
      }) =>
        chartData.t?.map((t: number, idx: number) => {
          return {
            timestamp: t * 1000,
            open: chartData.o[idx],
            high: chartData.h[idx],
            low: chartData.l[idx],
            close: chartData.c[idx],
            volume: chartData.v[idx],
          }
        }) || []
    )
  }

  /**
   * Subscribe to real-time data of the symbol in a certain period
   * Triggered when the symbol and period change
   *
   * Notify chart to receive data through callback
   */
  subscribe(symbol: SymbolInfo, period: Period, callback: DatafeedSubscribeCallback): void {
    if (!symbol.ticker) {
      return
    }

    if (
      this._prevData &&
      (this._prevData?.symbol?.market !== symbol.market || this._prevData?.period !== period)
    ) {
      this._onUnsubscribe?.(this._prevData.symbol.ticker, this._prevData?.period.multiplier)
    }

    this._onSubscribe?.(period, callback)
    this._prevData = { symbol, period }
  }

  /**
   * Unsubscribe to real-time data of the symbol in a certain period
   * Triggered when the symbol and period change
   *
   */
  unsubscribe(symbol: SymbolInfo, period: Period): void {
    this._onUnsubscribe?.(symbol.ticker, period.multiplier)
  }
}
