...
function Recipe() {
  ...
  const { nextRecipeState, changedKeys } = useMemo(() => {
    const updatedRecipe: Recipe = { ...recipe };
    const keys: string[] = [];

    if (agentState?.recipe) {
      const recipeKeys = Object.keys(recipe) as (keyof Recipe)[];
      for (const key of recipeKeys) {
        const agentValueRaw = agentState.recipe[key];
        if (agentValueRaw === undefined || agentValueRaw === null) {
          continue;
        }

        const agentValue =
          typeof agentValueRaw === "string" ? agentValueRaw.replace(/\\n/g, "\n") : agentValueRaw;
        const currentValue = recipe[key];

        if (JSON.stringify(agentValue) !== JSON.stringify(currentValue)) {
          switch (key) {
            case "title":
              updatedRecipe.title = agentValue as string;
              break;
            case "skill_level":
              updatedRecipe.skill_level = agentValue as SkillLevel;
              break;
            case "cooking_time":
              updatedRecipe.cooking_time = agentValue as CookingTime;
              break;
            case "special_preferences":
              updatedRecipe.special_preferences = agentValue as string[];
              break;
            case "ingredients":
              updatedRecipe.ingredients = agentValue as Ingredient[];
              break;
            case "instructions":
              updatedRecipe.instructions = agentValue as string[];
              break;
            default: {
    ...
    return { nextRecipeState: updatedRecipe, changedKeys: keys };
  }, [agentState, recipe]);

  useEffect(() => {
    if (changedKeys.length > 0) {
      changedKeysRef.current = changedKeys;
      setRecipe(nextRecipeState);
    } else if (!isLoading) {
      changedKeysRef.current = [];
    }
  }, [changedKeys, isLoading, nextRecipeState, setRecipe]);
