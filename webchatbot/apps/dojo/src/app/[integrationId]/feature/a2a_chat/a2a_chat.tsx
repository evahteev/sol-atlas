...
const Chat = ({ onNotification }: { onNotification?: () => void }) => {
  ...
  useEffect(() => {
    ...
    const lastMessage = visibleMessages[visibleMessages.length - 1] as { name?: string } | undefined;

    if (!isLoading || lastMessage?.name === "pickTable") {
      console.log("onNotification");
      onNotification?.();
    }
  }, [isLoading, onNotification, visibleMessages]);
